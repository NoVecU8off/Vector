...
pub async fn sign_block(block: &Block, keypair: &NodeKeypair) -> Result<Signature, BlockOpsError> {
    let hash = hash_header_by_block(block)?;
    let signature = keypair.sign(&hash);
    Ok(signature)
}

pub async fn verify_block(block: &Block, signature: &Signature, keypair: &NodeKeypair) -> Result<bool, BlockOpsError> {
    let hash = hash_header_by_block(block)?;
    Ok(keypair.verify(&hash, signature))
}

pub fn verify_block_sync(block: &Block, signature: &Signature, keypair: &NodeKeypair) -> Result<bool, BlockOpsError> {
    let hash = hash_header_by_block(block)?;
    Ok(keypair.verify(&hash, signature))
}
