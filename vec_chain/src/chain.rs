...
impl HeaderList {
    ...
    pub fn get_header_by_index(&self, index: usize) -> Result<&Header, ChainError> {
        if index >= self.headers_list_height() {
            return Err(ChainError::IndexTooHigh);
        }
...
impl Chain {
    pub async fn new_chain(block_store: Box<dyn BlockStorer>) -> Result<Chain, ChainError> {
        let mut chain = Chain {
            block_store,
    ...
    pub async fn validate_block(&self, incoming_block: &Block) -> Result<(), ChainError> {
    ...
    pub async fn add_block(&mut self, block: Block) -> Result<(), ChainError> {
        let header = block
            .msg_header
            .as_ref()
            .ok_or(ChainError::MissingBlockHeader)?;
    ...
    pub async fn add_leader_block(&mut self, block: Block) -> Result<(), ChainError> {
        let header = block
            .msg_header
            .as_ref()
            .ok_or(ChainError::MissingBlockHeader)?;
    ...
    pub async fn get_block_by_hash(&self, hash: &[u8]) -> Result<Block, ChainError> {
        let hash_hex = encode(hash);
        match self.block_store.get(&hash_hex).await {
            Ok(Some(block)) => Ok(block),
            Ok(None) => Err(ChainError::BlockNotFound(hash_hex)),
            Err(err) => Err(err.into()),
        }
    }
    
    pub async fn get_block_by_height(&self, height: usize) -> Result<Block, ChainError> {
        if self.chain_len() == 0 {
            return Err(ChainError::ChainIsEmpty);
        }
        if self.chain_height() < height {
            return Err(ChainError::HeightTooHigh(height, self.chain_height()));
        }
    ...
    async fn check_block_signature(&self, incoming_block: &Block) -> Result<(), ChainError> {
    ...
    async fn check_previous_block_hash(&self, incoming_block: &Block) -> Result<(), ChainError> {
        if self.chain_len() > 0 {
            ...
            if let Some(header) = incoming_block.msg_header.as_ref() {
                if last_block_hash != header.msg_previous_hash {
                    return Err(ChainError::InvalidPreviousBlockHash {
                        0: encode(last_block_hash),
                        1: encode(header.msg_previous_hash.clone()),
                    });
                }
            } else {
                return Err(ChainError::MissingBlockHeaderInIncomingBlock);
            }
        }
        Ok(())
    }

    pub async fn get_previous_hash_in_chain(&self) -> Result<Vec<u8>, ChainError> {
        let last_block = self.get_block_by_height(self.chain_height()).await?;
        let last_block_hash = hash_header_by_block(&last_block)?.to_vec();
        Ok(last_block_hash)
    }

    fn check_transactions_in_block(&self, incoming_block: &Block) -> Result<(), ChainError> {
        incoming_block
            .msg_transactions
            .par_iter()
            .try_for_each(|tx| {
    ...
    fn validate_transaction(&self, transaction: &Transaction) -> Result<(), ChainError> {
    ...
    fn extract_public_keys_from_transaction(&self, transaction: &Transaction) -> Result<Vec<PublicKey>, ChainError> {
        let mut public_keys = Vec::new();
        for input in &transaction.msg_inputs {
            let public_key = PublicKey::from_bytes(&input.msg_public_key)
                .map_err(|_| ChainError::InvalidPublicKeyInTransactionInput)?;
            public_keys.push(public_key);
        }
        Ok(public_keys)
    }

    fn check_transaction_signature(&self, transaction: &Transaction, public_keys: &[PublicKey]) -> Result<(), ChainError> {
        if !verify_transaction(transaction, public_keys) {
            Err(ChainError::InvalidTransactionSignature)
        } else {
            Ok(())
        }
    }
...
pub async fn create_genesis_block() -> Result<Block, ChainError> {
    ...
    let signature = sign_block(&block, &genesis_keypair).await.map_err(|e| ChainError::GenesisBlockCreationFailed(Box::new(e)))?;
